...
table.insert(M, {
	...
	opts = {
		...
		picker = {
			...
			sources = {
				select = {
					-- `reverse = false` matches the other compact popups (`gs`, `ss`):
					-- the select preset puts the input on top, so results read top-down.
...
table.insert(M, {
	...
	keys = {
		{ '<leader>fi', '<cmd>Nerdy list<CR>', desc = 'Browse nerd icons' },
		{ '<leader>fI', '<cmd>Nerdy recents<CR>', desc = 'Browse recent nerd icons' },
	},
})
